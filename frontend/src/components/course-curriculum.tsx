...
export function CourseCurriculum({ content }: { content: CourseContent }) {
  ...
  return (
    <section aria-labelledby="course-curriculum" className="mt-8 scroll-mt-24 sm:mt-10">
      <div className="flex flex-col items-start gap-2 border-b border-gw-border-soft pb-4 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
        <div>
          <div className="font-mono text-[10px] tracking-[0.14em] uppercase text-gw-text-faint">Course curriculum</div>
          <h2 id="course-curriculum" className="mt-1.5 font-serif text-[26px] tracking-tight text-gw-ink sm:text-[30px]">
      ...
      <div className="mt-5 overflow-hidden rounded-2xl border border-gw-border-soft bg-gw-surface shadow-[0_8px_24px_-18px_rgba(28,30,42,0.25)]">
        {content.sections.map((section, index) => (
          <details key={section.title} open={index === 0} className="group border-b border-gw-border-hairline last:border-0">
            <summary className="flex cursor-pointer list-none items-center gap-3 px-4 py-3.5 marker:content-none hover:bg-gw-surface-muted sm:gap-4 sm:px-5 sm:py-4">
              ...
              <span className="hidden shrink-0 font-mono text-[10.5px] tracking-wide text-gw-text-faint min-[480px]:block">
            ...
            <div className="border-t border-gw-border-hairline bg-gw-surface-muted px-4 py-4 sm:pl-[4.75rem]">
              ...
              <ol className="mt-3.5 divide-y divide-gw-border-hairline">
                {section.lessons.map((lesson, lessonIndex) => (
                  <li key={lesson.title} className="flex items-center gap-2.5 py-2.5 text-[13.5px] text-gw-text">
                    <span className="font-mono text-[10px] text-gw-text-placeholder">{String(lessonIndex + 1).padStart(2, "0")}</span>
                    <span className="flex-1">{lesson.title}</span>
                    <span className="shrink-0 font-mono text-[10.5px] text-gw-text-faint">{lesson.duration_label}</span>
                  </li>
                ))}
              </ol>
            </div>
