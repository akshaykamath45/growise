...
export default function CatalogHealthPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMessage("Catalog health could not be loaded. Check the vector store connection and try again.");
    } finally {
  ...
  if (initialLoading) return <LoadingState title="Checking catalog health" description="Verifying SQL records and vector retrieval readiness." />;
  if (!health) return <LoadingState title="Catalog health unavailable" description={message || "Trying to reach the catalog health service."} />;
  const attention = health.pending_sync_courses + health.failed_sync_courses;
