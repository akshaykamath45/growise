  ...
  return (
    <article className="rounded-2xl border border-gw-border-hairline bg-gw-surface-muted p-4 sm:p-5">
      ...
      {step.step_name === "rerank_candidates" && (
        <div className="mt-4"><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-mono text-[9px] tracking-[0.12em] uppercase text-gw-text-faint">Mesh fit & diversity pass</div>{output.rerank_fallback ? <span className="rounded-full border border-gw-agent-border bg-gw-agent-bg px-2 py-0.5 font-mono text-[9px] text-gw-agent">vector-order fallback</span> : <span className="text-[10px] text-gw-text-faint">semantic fit first · category variety surfaced</span>}</div><div className="mt-2 divide-y divide-gw-border-hairline rounded-xl border border-gw-border-hairline bg-gw-surface">{reranked.map((item, index) => { const id = Number(item.product_id); const picked = selectedIds.has(id); return <div key={String(item.product_id)} className="flex items-start justify-between gap-3 px-3 py-2.5"><div><div className="text-[12px] font-medium text-gw-ink">{index + 1}. {asText(item.title) || `Course #${id}`}</div><div className="mt-1 flex flex-wrap gap-x-2 font-mono text-[9.5px] text-gw-text-faint"><span>{Number(item.fit_score ?? 0).toFixed(0)} fit</span><span>{asText(item.learning_role) || "next step"}</span></div>{asText(item.rationale) && <p className="mt-1 text-[11px] leading-relaxed text-gw-text-muted">{asText(item.rationale)}</p>}</div><span className={`mt-0.5 rounded-full px-2 py-0.5 font-mono text-[9px] ${picked ? "bg-gw-agent-bg text-gw-agent" : "bg-gw-surface-muted text-gw-text-faint"}`}>{picked ? "final pick" : "available"}</span></div>; })}</div></div>
      )}
